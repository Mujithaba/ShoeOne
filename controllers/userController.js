...
//     sendMail(otp, req.session.email, req.session.Firstname);
//     res.render("user/OTPverify", {
//       msg: "Resend otp is passed,check your mail",
//     });
//   } catch (error) {
//     console.log(error.message);
//   }
      ...
      if (passwordMatch) {
        if (userData.is_blocked == false) {
          if (userData.is_verified == 1) {
            ...
            res.render("user/login", {
              verify: "Your are not verified, Verify before login or Register again",
            });
          }
        } else {
          res.render("user/login", {
            message: "Admin is blocked you ,So you can't login",
          });
        }
      } else {
...
const settingForgotPass = async (req, res) => {
  try {
    ...
    if (setPassword) {
      ...
      res.render("user/forgetPassword", {
        message: "something went wrong when you are changing password.",
      });
    }
  } catch (error) {
    console.log(error.message);
...
const loadShop = async (req, res) => {
  try {
    ...
    const fullProductsCount = await Product.find(
      productsQuery
    ).countDocuments();
...
const productDetailed = async (req, res) => {
  try {
    ...
    const findProduct = await Product.findOne({ _id: productid }).populate(
      "category"
    );
